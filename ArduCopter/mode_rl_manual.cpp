// mode_rl_manual.cpp
// This file contains the main code for running RL manual with a Jetson TX2 as the upper computer.
...
bool ModeRL_Manual::init(bool ignore_checks)
{
    ...
    switch(fsm.cur_state){
        case ARMED:
            if(fsm.cur_state != fsm.prev_state){
                ...
                copter.mode_RL_Manual.fsm.target_duration = g.disarm_delay; // the disarm delay is the same as system configures
        ...
        case DISARMED:
            if(fsm.cur_state != fsm.prev_state){
                ...
                copter.mode_RL_Manual.fsm.begin = now;
                gcs().send_text(MAV_SEVERITY_INFO, "Current state start time is %.3f\n",copter.mode_RL_Manual.fsm.begin);
        ...
        case TAKEOFF:
            if(fsm.cur_state != fsm.prev_state){
                ...
                printf("Height: %f\n", copter.mode_RL_Manual.fsm.params[0]);
                copter.mode_RL_Manual.fsm.begin = now;
                gcs().send_text(MAV_SEVERITY_INFO, "Current state start time is %.3f\n",copter.mode_RL_Manual.fsm.begin);

                // save the duration of takeoff, which doubles the takeoff height
                copter.mode_RL_Manual.fsm.target_duration = 2 * fsm.params[0]; 
            ...
            else // continuing with TAKEOFF procedure
            {
                ...
                int thrust_p_takeoff = int((copter.mode_RL_Manual.fsm.target_duration*1000000.0-timeInThisRun)*0.00001); //5s takeoff, max thrust = 5*10^6*0.00001
        ...
        case HOVER:
            if(fsm.cur_state != fsm.prev_state){
                ...
                copter.mode_RL_Manual.fsm.target_duration = 10000; // seconds
        ...
        case LAND:
            ...
            else // continuing with LAND procedure
            {
                ...
                {
                    int thrust_p_landing = int((copter.mode_RL_Manual.fsm.target_duration*1000000.0-timeInThisRun)*0.00001); //5landing p control
...
void ModeRL_Manual::determine_states(float currentTime){
...
#if (USE_BATT_COMP)
VectorN<float, 4> ModeRL_Manual::motorMixingBattComp(VectorN<float, 4> thrustMomentCmd, float BattVolt)
{
    #if (REAL_OR_SITL!=0)
...
#else
VectorN<float, 4> ModeRL_Manual::motorMixing(VectorN<float, 4> thrustMomentCmd)
{
    VectorN<float, 4> w;
#if (REAL_OR_SITL==0)       // SITL
    const float L = 0.25; // for x layout
...
VectorN<float, 4> ModeRL_Manual::iterativeMotorMixing(VectorN<float, 4> w_input, VectorN<float, 4> thrustMomentCmd, float a_F, float b_F, float a_M, float b_M, float L, float D)
{
...
VectorN<float, 16> ModeRL_Manual::mat4Inv(VectorN<float, 4> coefficientRow1, VectorN<float, 4> coefficientRow2, VectorN<float, 4> coefficientRow3, VectorN<float, 4> coefficientRow4)
{
